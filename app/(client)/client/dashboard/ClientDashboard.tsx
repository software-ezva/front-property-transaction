...
export default function ClientDashboard() {
  ...
  return (
    <DashboardLayout user={clientUserForHeader}>
        ...
        <div className="bg-gradient-to-r from-primary/10 to-secondary/10 rounded-lg p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-sm">
            <span className="bg-primary/20 text-primary px-3 py-1 rounded-full flex items-center">
              ...
              {clientProfile.preferred_locations?.join(", ")}
            </span>
            <span className="bg-secondary/20 text-secondary px-3 py-1 rounded-full">
              Budget: ${clientProfile.budget_range?.min?.toLocaleString()} - $
              {clientProfile.budget_range?.max?.toLocaleString()}
            </span>
          </div>
        </div>
